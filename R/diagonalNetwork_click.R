...
#' @export
#'
diagonalNetwork_click <- function(
                          List,
                          height = NULL,
                          width = NULL,
                          fontSize = 10,
...
#' @export
diagonalNetwork_clickOutput <- function(outputId, width = "100%", height = "800px") {
    shinyWidgetOutput(outputId, "diagonalNetwork_click", width, height,
                        package = "networkD3")
}

#' @rdname networkD3-shiny
#' @export
renderdiagonalNetwork_click <- function(expr, env = parent.frame(), quoted = FALSE) {
    if (!quoted) { expr <- substitute(expr) } # force quoted
    shinyRenderWidget(expr, diagonalNetwork_clickOutput, env, quoted = TRUE)
}
